...
public:
   ...
   // Image information
   int getScanOption() const;                      ///< Determine the way the input pixel data must be scanned to accommodate the required rotate and flip options.
   void getPixelTranslation();                     ///< Generate a lookup table to convert raw pixel values to display pixel values
   unsigned int maxPixelValue() const;             ///< Determine the maximum pixel value for the current format
   unsigned int rotatedImageBuffWidth() const;     ///< Return the image width following any rotation
   unsigned int rotatedImageBuffHeight() const;    ///< Return the image height following any rotation
   imageDisplayProperties::rgbPixel
   getFalseColor (const unsigned char value) const; ///< Get a false color representation for an entry fro the color lookup table
   int getElementCount() const;                                                   ///< Determine the element count expected based on the available dimensions
   bool validateDimensions();                                                     ///< Determine if the image dimensional information is valid.
   void getPixelRange( const QRect& area,
                       unsigned int& min,  unsigned int& max ) const;             ///< Determine the range of pixel values an area of the image
   bool hasImage() const { return !imageData.isEmpty(); }                         ///< Return true if the current image is empty
   const unsigned char* getImageDataPtr( const QPoint& pos ) const;               ///< Return a pointer to pixel data in the original image data.
   int getPixelValueFromData( const unsigned char* ptr ) const;                   ///< Return a number representing a pixel intensity given a pointer into an image data buffer.
   double getFloatingPixelValueFromData( const unsigned char* ptr ) const;        ///< Return a floating point number representing a pixel intensity given a pointer into an image data buffer.

   QImage copyImage() const;         ///< Return a QImage based on the current image
   ...
   void generateProfileData( QVector<QPointF>& profileData,
                             QPoint point1, QPoint point2, unsigned int thickness );           ///< Generate a series of pseudo pixel values from an arbitrary line between two pixels.

   // Transformations
   QRect rotateFlipToDataRectangle( const QRect& rect ) const;                 ///< Transform a rectangle from the image to the original data according to current rotation and flip options
   QRect rotateFlipToDataRectangle( const QPoint& pos1,
                                    const QPoint& pos2 ) const;                ///< Transform a rectangle from the image to the original data according to current rotation and flip options
   QPoint rotateFlipToDataPoint( const QPoint& pos ) const;                    ///< Transform a point from the image to the original data according to current rotation and flip options.
